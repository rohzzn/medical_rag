...
class RagPipeline:
    def __init__(self):
        ...
        # First check if we can load the real thing
        try:
            # Only import if OpenAI API key is available
        ...
        ''', 
                ...
                print("OpenAI API key not found, RAG pipeline disabled")
    ...
    def search(self, query: str, conversation_history=None) -> Dict[str, Any]:
        ...
        if not self.rag_enabled:
            # Return a fallback response if RAG is not enabled
            return {
                "answer": "I'm sorry, but the RAG pipeline is currently unavailable. Please make sure your OpenAI API key is properly configured.",
                "sources": []
            }
            
        try:
            # Import here to avoid errors if RAG is not enabled
            from neo4j_graphrag.retrievers import HybridCypherRetriever
            from neo4j_graphrag.generation.graphrag import GraphRAG
            from app.rag.neo4j import Neo4jManager
            ...
            return {
                "answer": f"I encountered an error while searching: {str(e)}. Please check your configuration and try again.",
                "sources": []
            }
